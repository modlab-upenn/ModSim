...
def test_runtime_matches_planar_layout_with_live_target_planning_and_event_tabs(
    ...
    try:
        ...
        for theme in THEMES:
            ...
            app.processEvents()
            assert window.spatial_planning.action_table.rowCount() == 1
            assert window.spatial_planning.decisions.rowCount() == 1
            assert window.spatial_planning.history.plot.getViewBox().state["mouseEnabled"] == [
                True,
                False,
            ]
            window.spatial_planning.projection.setCurrentIndex(1)
            assert window.spatial_planning.project((0.0, 0.0, 0.15)) == (0.0, 0.15)
            window.spatial_planning.workspace.setXRange(-0.1, 0.1, padding=0)
            before = window.spatial_planning.workspace.viewRange()
            window.spatial_planning.set_frame(initial)
            assert window.spatial_planning.workspace.viewRange() == before
        ...
        stale = initial.spatial_planning.model_copy(update={"sample_sequence": 999})
        bad = initial.model_copy(update={"spatial_planning": stale})
