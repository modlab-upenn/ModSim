...
MuJoCo ships no type stubs, so `src/modsim_backend_mujoco` is excluded from the
strict pyright configuration in `pyproject.toml` and checked separately by
`pyright-mujoco.json` at standard strictness. This mirrors how the Qt-heavy
Studio modules are handled.
