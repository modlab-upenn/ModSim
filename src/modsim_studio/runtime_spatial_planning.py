...
class SpatialPlanningWorkspace(QWidget):
    ...
    def set_frame(self, frame: RuntimeInspectorFrame) -> None:
        if frame.spatial_planning is None:
            return
        self._frame = frame
    ...
    def _redraw(self) -> None:
        frame = self._frame
        if frame is None or frame.spatial_planning is None:
            return
        snapshot = frame.spatial_planning
    ...
    def _update_detail(self) -> None:
        if self._frame is None or self._frame.spatial_planning is None:
            return
        row = self.action_table.currentRow()
        actions = self._frame.spatial_planning.actions
