...
class RuntimeInspectorRunner:
    ...
    def execution_finished(self) -> bool:
        ...
        return self.scenario.status.phase.value in {"complete", "failed", "stopped", "timed_out"}
        ...
        if isinstance(self.scenario, SpatialReconfigurationScenario):
            ...
            for _ in range(round(self.config.dt_s / self.scenario.dt_s)):
                ...
                if self.scenario.terminal:
                    break
            if (
                not self.scenario.terminal
                and self.session.world.time_s - self.scenario.started_at
                >= self.scenario.duration_s - 1e-10
            ):
                events.extend(self.scenario.step())
            return tuple(events)
    ...
    def frame(self) -> RuntimeInspectorFrame:
        ...
        frame = build_runtime_inspector_frame(
            ...
            spatial_planning=self.scenario.planning_snapshot()
            if isinstance(self.scenario, SpatialReconfigurationScenario)
            else None,
