...
RuntimeModelView: TypeAlias = ModuleTopologyGraphView | CubicLatticeView
...
class RuntimeInspectorFrame(_RuntimeInspectionDTO):
    ...
    spatial_planning: SpatialPlanningSnapshot | None = None
    ...
    def require_contiguous_event_delta(self) -> RuntimeInspectorFrame:
        ...
        planners = [
            p
            for p in (self.planning, self.lattice_planning, self.spatial_planning)
            if p is not None
        ]
        if len(planners) > 1:
...
def build_runtime_inspector_frame(
    ...
    *,
    ...
    if not isinstance(generated, ModuleTopologyGraphView | CubicLatticeView):
        raise RuntimeInspectionError(
            "Runtime Inspector has no renderer for "
            f"view type '{generated.view_type}' generated by recipe '{recipe.id}'"
        )
...
def format_event_detail(event: Event) -> str:
    ...
    if isinstance(event, DockCommitted):
        ...
        return (
            f"{event.connector_a} <-> {event.connector_b} ({event.constraint.value}, {orientation})"
        )
