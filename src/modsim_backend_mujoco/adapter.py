...
This milestone implements the observation half of the backend contract: load a
scene, step real physics, and report link poses, twists, connector frames, and
time. Runtime docking is not wired yet, and the adapter says so through its
capabilities rather than pretending otherwise, so the docking engine refuses to
commit connections it cannot have enforced.
"""
...
class MuJoCoBackendAdapter:
    ...
    def _snap_to_nominal(
        ...
        self.set_module_pose(request.module_b, target)
        mujoco.mj_forward(model, data)
