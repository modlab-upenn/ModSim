...
class CompiledScene:
    ...
    here so that enabling docking later needs no change to scene construction.
    """
...
def _add_connector_sites(
    ...
    for module_id, module_type in module_types.items():
        for connector in module_type.connectors:
            ...
            local = (
                Transform.from_pose_spec(connector.local_pose)
                if connector.local_pose is not None
...
def _reserve_weld_pool(
    ...
    """Pre-allocate inactive weld constraints for future runtime docking."""
